...
The assets primarily affected are those having ```CMD``` or ```TR``` suffixes - so this excludes Forex, Crypto, Equities - equities are a different kind of animal dividend/splits - since they are Spot/Cash based. Indices rollover quarterly, I have identified rollovers in US30 as well.
