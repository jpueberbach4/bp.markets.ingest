...
def load_symbols() -> pd.Series:
    ...
    Converts symbols to strings and replaces '/' with '-' to standardize
    naming for file paths.
...
def download_symbol(symbol: str, dt: date) -> None:
    ...
    try:
        ...
        try:
            ...
            tqdm.write(f"Connection error occurred for URL: {url}")
        except requests.exceptions.HTTPError as e:
            tqdm.write(f"HTTP error: {e} - URL: {url}")
        except requests.exceptions.RequestException as e:
            tqdm.write(f"Unexpected error: {e} - URL: {url}")
        else:
