...
class SingularityInference(nn.Module):
    """1D-CNN Inference Architecture mirroring RPulsarCore Macro-Stencil."""
    def __init__(self, gene_count: int, hidden_dim: int, kernel_size: int, flatten_dim: int):
        super(SingularityInference, self).__init__()
        self.conv = nn.Conv1d(in_channels=gene_count, out_channels=hidden_dim, kernel_size=kernel_size)
        self.activation = nn.GELU() 
        self.fc = nn.Linear(flatten_dim, 1)
        self.out_act = nn.Sigmoid()
        
    def forward(self, x):
        # x shape: (Num_Windows, Gene_Count, Lookback)
        c1 = self.conv(x)
        a1 = self.activation(c1)
        
        # Flatten time and feature dimensions for final scoring
...
def get_cached_model(checkpoint_path: str, device: torch.device):
    ...
    # Extract CNN dimensions
    hidden_dim, gene_count, kernel_size = w1.shape
    config = checkpoint.get('config', {})
    lookback = int(config.get('lookback', 24))
    
    # In the Macro-Stencil setup, Kernel_Size == Lookback, so flatten_dim is just hidden_dim
...
def calculate(df: pd.DataFrame, options: Dict[str, Any]) -> pd.DataFrame:
    ...
    # This completely overrides the stale scaler from the .pt file.
    # =====================================================================
    ...
    # =====================================================================
    # 1D-CNN SEQUENCE WINDOWING
    # =====================================================================
    if lookback > 1:
        if len(normalized_tensor) < lookback:
            return pd.DataFrame({'score': 0.0, 'signal': 0.0}, index=df.index)
        
        # Unfold shifts the sequence into blocks: (Num_Windows, Gene_Count, Lookback)
        # Note: Unfold on dim 0 preserves the spatial (Gene) dimension untouched
        inference_tensor = normalized_tensor.unfold(dimension=0, size=lookback, step=1)
    else:
        inference_tensor = normalized_tensor.unsqueeze(-1) # Fallback to pseudo-sequence

    with torch.no_grad():
        # RUN INFERENCE ON THE HISTORICAL SEQUENCE BLOCKS
        ...
        if (current_time - cached_data['last_audit_time']) > 30:
            ...
            # CNN Feature Impact Calculation: Sum absolute weights across all filters and time kernels
            # w1 shape: (Filters, Genes, Time) -> sum over 0 and 2 isolates the Gene impact
