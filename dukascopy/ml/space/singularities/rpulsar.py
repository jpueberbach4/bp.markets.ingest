...
from ml.space.singularities.cores.rpulsar_core import RPulsarCore

class RPulsarSingularity(Singularity):
    ...
    def __init__(self, config):
        ...
        self.core = RPulsarCore(config, self.device, self.config.get('seed', 42))
    ...
    def run_generation(self, config):
        ...
        for i in range(0, self.core.pop_size, self.chunk_size):
            ...
            # Haal de pure data op: (Total_Train_Rows, Chunk_Size, Gene_Count)
            raw_x_train = self.lake[:train_end, indices] 
            
            # PyTorch Unfold Magie: Verschuivend window over Dimensie 0 (de tijd)
            # Dit transformeert de data naar: (Num_Windows, Chunk_Size, Gene_Count, Lookback)
            # OPMERKING: Omdat we 'lookback' candles nodig hebben voor de eerste voorspelling, 
            # verliezen we de allereerste (lookback - 1) targets.
            windows_x_train = raw_x_train.unfold(dimension=0, size=lookback, step=1)
            
            # Herschikken voor de Core Forward pass: 
            # We willen: (Chunk_Size, Num_Windows, Gene_Count * Lookback)
            # Permute: (Chunk_Size, Num_Windows, Gene_Count, Lookback)
            windows_x_train = windows_x_train.permute(1, 0, 2, 3) 
            
            # Platstrijken van de Feature + Lookback dimensies voor de Lineaire laag
            num_windows = windows_x_train.size(1)
            x_train = windows_x_train.reshape(curr_chunk, num_windows, -1) 
            
            # Pas y_train aan omdat we de eerste paar rijen missen door de lookback
