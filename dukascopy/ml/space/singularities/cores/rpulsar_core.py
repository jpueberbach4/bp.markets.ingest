...
class RPulsarCore(Fabric):
    ...
    def __init__(self, config, device, seed=42):
        ...
        # Hoeveel candles terugkijken voor de indicatoren (voor nu 5000, maar kan dynamisch worden aangepast)
    ...
    def init_population(self, num_indicators: int, feature_names: list):
        ...
        # W1 heeft nu een extra dimensie voor de 'lookback' (Time)
        # We bouwen eigenlijk een spatio-temporele filter: (Gene_count * Lookback)
        self.pop_W1 = (
    ...
    def forward(self, x, w1, b1, w2, b2):
        """
        Performs the Recurrent/Temporal Neural Network Forward Pass.
        Verwacht dat 'x' een shape heeft van: (Population_Chunk, Time_Steps, Gene_Count)
        """
        # h1 berekent de contextuele relatie tussen de features over de lookback tijd
        h1 = torch.bmm(x, w1)
        h1 = F.gelu(h1 + b1)
    ...
    def evolve(self, latest_f1, latest_score):
        ...
        for i in range(keep, self.pop_size):
            ...
            if torch.rand(1, generator=self.torch_generator).item() < 0.7:
                ...
                # De W1 cut moet vermenigvuldigd worden met de lookback
                w1_cut = cut * self.lookback
                
                # Genes and W1 weights must stay exactly together (in blokken van lookback)
                ...
                if len(unique_genes) < self.gene_count:
                    ...
                    # Initialize completely fresh, decoupled weights (MET LOOKBACK)
                    final_w1 = torch.randn((self.gene_count * self.lookback, self.hidden_dim), device=self.device) * 0.01
                    ...
                    for j, gene in enumerate(unique_genes):
                        ...
                        # Kopieer het hele tijdsvenster (blok van lookback grootte)
                ...
                if torch.rand(1, generator=self.torch_generator).item() < 0.3:
                    ...
                    for _ in range(n_mutations):
                        ...
                        if len(available) > 0:
                            ...
                            # Reset het specifieke tijdsblok voor dit ene gen
                            pos_start = pos * self.lookback
                            pos_end = (pos + 1) * self.lookback
                            child_w1[pos_start:pos_end] = torch.randn((self.lookback, self.hidden_dim), device=self.device) * 0.01
    ...
    def run_atomic_scan(self):
        ...
        for i in range(start_idx, self.pop_size):
            ...
            self.pop_W1[i] = (
                torch.randn(self.gene_count * self.lookback, self.hidden_dim, device=self.device, generator=self.cuda_generator)
                * np.sqrt(2.0 / (self.gene_count * self.lookback))
            )
    ...
    def emit(self, features: pd.DataFrame) -> np.ndarray:
        """Inference wrapper for the best individual, with Recurrent Windowing."""
        if len(features) < self.lookback:
            return np.array([False] * len(features)) # Niet genoeg data voor 1 lookback window
            
        with torch.no_grad():
            x = torch.tensor(features.values.astype(np.float32), device=self.device)
            # Selecteer de winnende genen: (Totale_Lengte, Gene_Count)
            x_sel = x[:, self.population[0]] 
            
            # Pas de sliding window toe: (Num_Windows, Gene_Count, Lookback)
            windows = x_sel.unfold(dimension=0, size=self.lookback, step=1)
            
            # Voeg een dummy Batch dimensie toe en vlak features/tijd af
            # Verwachte shape voor forward: (1, Num_Windows, Gene_Count * Lookback)
            ...
            # Omdat we (lookback - 1) rijen verliezen aan het begin door het window, 
            # pad de output met 'False' aan het begin om de originele dataframe lengte te behouden
