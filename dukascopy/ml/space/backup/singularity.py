...
class EventHorizonSingularity(Singularity):
    ...
    def run_generation(self, universe):
        chunk_size = self.config.get("GPU_CHUNK", 512)
        min_sigs = self.config.get("MIN_SIGNALS", 3)
        target_density = self.config.get("TARGET_DENSITY", 0.01)
        precision_exp = self.config.get("PRECISION_EXP", 2.5) 
        penalty_coeff = self.config.get("PENALTY_COEFF", 1.0)
        thresh_steps = self.config.get("THRESH_STEPS", 31)
        verbose = self.config.get("VERBOSE", True)
        
        metrics = {"f1": [], "sigs": [], "density": [], "precision": [], "recall": [], "score": []}
        train_end = int(len(self.lake) * self.config.get("OOS_BOUNDARY", 0.75))
        ...
        for i in range(0, self.pop_size, chunk_size):
            ...
            for _ in range(self.config.get("EPOCHS", 25)):
                optimizer.zero_grad()
    ...
    def evolve(self, fitness_scores):
        ...
        mutation_std = self.config.get("WEIGHT_MUTATION_RATE", 0.005)
